...
wget $s6_noarch_url -O /tmp/s6-overlay-noarch.tar.xz && \
...
wget $s6_symlinks_noarch_url -O /tmp/s6-overlay-symlinks-noarch.tar.xz && \
tar -C / -Jxpf /tmp/s6-overlay-symlinks-noarch.tar.xz && \
rm /tmp/s6-overlay-symlinks-noarch.tar.xz && \

echo "Downloading from $s6_symlinks_arch_url" && \
wget $s6_symlinks_arch_url -O /tmp/s6-overlay-symlinks-arch.tar.xz && 
